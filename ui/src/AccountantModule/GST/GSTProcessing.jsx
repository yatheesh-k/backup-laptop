...
const GSTProcessing = () => {
  ...
  const calculateTotalGST = () => {
    return approvalList.reduce((total, invoice) => {
      const gstAmount = parseFloat(invoice.totalAmount) || 0;
  ...
  const downloadApprovalListExcel = () => {
    ...
    const formattedData = approvalList.map(invoice => ({
      ...
      "Total Amount": invoice.totalAmount ? (invoice.totalAmount) : "N/A",
      "Sub Total": invoice.subTotal ? (invoice.subTotal) : "N/A",
      "IGST": invoice.igst ? (invoice.igst) : "N/A",
      "CGST": invoice.cgst ? (invoice.cgst) : "N/A",
      "SGST": invoice.sgst ? (invoice.sgst) : "N/A",
      "Month": invoice.month,
      "Year": invoice.year,
    }));
                          ...
                          <tbody>
                            {approvalList.map((invoice, i) => (
                              <tr key={i}>
                                ...
                                <td>{invoice.igst ? (invoice.igst) : "N/A"}</td>
                                <td>{invoice.cgst ? (invoice.cgst) : "N/A"}</td>
                            ...
                            {approvalList.length > 0 && (
                              <>
                                <tr className="fw-bold">
                                  <td colSpan="3" className="text-end">Total GST Amount</td>
                                  <td>{calculateTotalGST().toFixed(2)}</td>
